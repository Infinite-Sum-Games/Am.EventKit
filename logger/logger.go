...
func (l *LoggerService) enrich(c *gin.Context, e *zerolog.Event) *zerolog.Event {
	...
	if len(pathParams) > 0 {
		event = event.Interface("path-params", pathParams)
	}
	if len(queryParams) > 0 {
		event = event.Interface("query-params", queryParams)
	}

	// Add headers information for debugging (in non-production)
	if l.Env != "PRODUCTION" {
...
func (l *LoggerService) Warn(msg string) {
	l.Logger.WithLevel(zerolog.WarnLevel).Msg(msg)
}

func (l *LoggerService) Error(msg string, err error) {
	l.Logger.WithLevel(zerolog.ErrorLevel).Err(err).Msg(msg)
}
...
func (l *LoggerService) LogMiddleware(c *gin.Context) {
	...
	if len(c.Params) > 0 {
		...
		requestData["path-params"] = pathParams
	}
	...
	if len(query) > 0 {
		...
		requestData["query-params"] = queryParams
	}
	...
	switch {
	case status >= 500:
		event = event.Str("error-type", "server-error")
	case status >= 400:
		event = event.Str("error-type", "client-error")
	case status >= 300:
		event = event.Str("redirect", "true")
	}

	// Log performance warnings for slow requests
	if duration > 5*time.Second {
		event = event.Str("performance-warning", "slow-request")
	}
