...
func (q *Queries) GetEventsByDateAndOrganizer(ctx context.Context, db DBTX, arg GetEventsByDateAndOrganizerParams) ([]GetEventsByDateAndOrganizerRow, error) {
	rows, err := db.Query(ctx, getEventsByDateAndOrganizer, arg.EventDate, arg.Abbr)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, name, department_name, email, password, phone_number, is_amrita_student, amrita_roll_number, college_name, college_city, academic_year, account_status, refresh_token FROM student
WHERE email = $1
`
